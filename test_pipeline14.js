...
(async () => {
  ...
  // Use evaluate to find the job and update status
  await page.evaluate(() => {
      // Find the ID from the UI by querying the list
      const jobHtml = document.getElementById('jobs-list').innerHTML;
      const match = jobHtml.match(/app\.openStatusModal\('([^']+)'\)/);
      if (match) {
          app.openStatusModal(match[1]);
      }
  });
  await page.waitForTimeout(500);
